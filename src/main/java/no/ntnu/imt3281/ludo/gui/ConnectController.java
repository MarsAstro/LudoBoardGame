...
public class ConnectController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		File[] files = { new File(detailsFileNames[0]), new File(detailsFileNames[1]), new File(detailsFileNames[2]) };

		if ((files[0].exists() && !files[0].isDirectory()) && (files[1].exists() && !files[1].isDirectory())
	...
	private boolean allFieldsValid() {
		return !(ipAddress.getText().isEmpty() || username.getText().isEmpty() || password.getText().isEmpty());
	}
	...
	public void handleServerLoginResponse(String ackMessage) {
		...
		case 1:
			Platform.runLater(() -> Client.getLudoController().userLoggedIn(username.getText()));
			closeWindow();
			break;
		case -1:
