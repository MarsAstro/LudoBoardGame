...
public class GameBoardController implements Initializable {
	...
	private void clickOnBoard(MouseEvent e) {
		...
		if (selectedToken != null) {
			...
			if (tileIndex != -1 && tileIndex != selectedTokenTilePos) {
				Client.sendMessage("Ludo.Move:" + gameID + "," + playerID + "," + selectedTokenTilePos + "," + tileIndex
						+ "," + selectedTokenPieceIndex);
	...
	private int checkForTile(double x, double y) {
		...
		for (int tileIndex = 0; tileIndex < points.size(); tileIndex++) {
			if (x >= points.get(tileIndex).getX() && x <= points.get(tileIndex).getX() + TILESIZE
					&& y >= points.get(tileIndex).getY() && y <= points.get(tileIndex).getY() + TILESIZE) {
				result = tileIndex;
				break;
			}
		}
	...
	private void fillPlayerHUDArrays() {
		...
		for (int i = 0; i < 4; ++i) {
			pieceImages.add(new Image(getClass().getResourceAsStream("/images/piece" + i + ".png")));
		}
	}
	...
	public void updatePiece(int playerID, int piece, int from, int to) {
		...
		if (to == 73 || to == 79 || to == 85 || to == 91) {
			...
			Point secondPoint = rand.nextInt(2) == 0 ? new Point(0, mapEdge) : new Point(mapEdge, mapEdge);
			path.getElements().add(new CubicCurveTo(firstPoint.getX(), firstPoint.getY(), secondPoint.getX(),
					secondPoint.getY(), goalX, goalY));
		} else {
