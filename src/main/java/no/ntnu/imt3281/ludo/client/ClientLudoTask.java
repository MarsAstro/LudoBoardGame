...
public class ClientLudoTask implements Runnable {
    ...
    private void handleReceivedLudoPacket(String message) {
        ...
        switch (tag) {
            ...
            case "JoinRandom:" :
                Platform.runLater(
                        () -> Client.ludoController.handleServerJoinRandomGame(ackMessage));
                break;
            case "Join:" :
                Platform.runLater(
                        () -> Client.ludoController.handleServerJoinChallengeGame(ackMessage));
                break;
            case "RandomSuccess:" :
    ...
    private void handleReceivedLudoChallengeTimedOutPacket(String ackMessage) {
        ...
        if (cc != null) {
            ...
            Client.getLudoController().resetChallengeController();
        } else if (challengeAlert != null) {
            Platform.runLater(() -> challengeAlert.close());
            Client.getLudoController().resetChallengeAlert();
        }
    }
